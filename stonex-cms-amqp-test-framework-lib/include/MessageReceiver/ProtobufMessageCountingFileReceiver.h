...
namespace stonex {
	namespace messaging {
		namespace test {
			...
			template <class T>
			class ProtobufMessageCountingFileReceiver : public MessageCountingFileReceiver
			{
			public:
				ProtobufMessageCountingFileReceiver(const FileMessageCountingReceiverConfiguration& config, CMSClientTestUnit & client_params, Notifier& parent)
					:MessageCountingFileReceiver(config, client_params, parent)
				{
				};
