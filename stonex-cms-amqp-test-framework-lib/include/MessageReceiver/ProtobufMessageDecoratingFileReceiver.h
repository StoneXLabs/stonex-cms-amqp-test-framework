...
namespace stonex {
	namespace messaging {
		namespace test {
			...
			template <class T>
			class ProtobufMessageDecoratingFileReceiver : public MessageDecoratingFileReceiver
			{
			public:
				ProtobufMessageDecoratingFileReceiver(const FileMessageDecoratingReceiverConfiguration& config, CMSClientTestUnit & client_params, Notifier& parent)
					:MessageDecoratingFileReceiver(config, client_params, parent)
				{
				};
